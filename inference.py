...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for ghostPos in self.allPositions:
            ...
            self.beliefs[ghostPos] = self.beliefs[ghostPos] * prob
        self.beliefs.normalize()
    ...
    def elapseTime(self, gameState):
        ...
        for ghostPos in self.allPositions:
            ...
            for newPos in newPosDist:
                newBelief[newPos] = newBelief[newPos] + newPosDist[newPos] * self.beliefs[ghostPos]
        newBelief.normalize()
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for i in range(self.numParticles):
            newParticle = self.legalPositions[i % len(self.legalPositions)]
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for ghostPos in beliefDist:
            beliefDist[ghostPos] = beliefDist[ghostPos] * self.getObservationProb(observation, pacPos, ghostPos, jailPos)
        if beliefDist.total() == 0:
            self.initializeUniformly(gameState)
        else:
            beliefDist.normalize()
            #resample from new belief distribution
            newSamples = []
            for i in range(self.numParticles):
                newSamples.append(beliefDist.sample())
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        random.shuffle(dist)
        for i in range(self.numParticles):
            self.particles.append(dist[i % len(dist)])
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for particle in self.particles:
            ...
            for i in range(self.numGhosts):
                weight = weight * self.getObservationProb(observation[i], pacPos, particle[i], self.getJailPosition(i))
            beliefDist[particle] = beliefDist[particle] + weight

        if beliefDist.total() == 0:
            self.initializeUniformly(gameState)
        else:
            beliefDist.normalize()
            newSamples = [beliefDist.sample() for i in range(self.numParticles)]
            self.particles = newSamples
